...
@triton.testing.perf_report(
    [
        triton.testing.Benchmark(
            ...
            x_vals=[768, 1024, 768 * 2, 2048, 768 * 3, 3072],  # different possible values for `x_name`
            ...
            plot_name="fista_num_basis",  # name for the plot. Used also as a file name for saving the plot.
            args={},  # values for function arguments not in `x_names` and `y_name`
        )
    ]
)
def benchmark(K, provider, **kwargs):
    set_seed(0)

    inp = torch.randn((512, 768), dtype=torch.float32, device='cuda')
    dictionary = torch.randn((768, K), dtype=torch.float32, device='cuda')
